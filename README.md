...
随着互联网上的图片，文本，视频信息日益膨胀，考虑到目前很多网民还是未成年人，因此检测其中的违规内容格外重要。本项目提供了部分违规内容的检测功能：违规图片检测（色情，恐暴）和违规文本检测（色情、辱骂、威胁等）。本项目不提供具体的数据和模型文件，不提供训练代码，仅提供基于TorchServe的推理后端和屏蔽复杂细节的Web后台。
...
* 对于色情、恐暴的违规图片，对于色情、辱骂、威胁等类别的违规文本拥有较准确的识别率
* 支持敏感词过滤功能，包含内置的词库文件且支持用户自定义添加敏感词
* 不仅返回模型的推理结果，而且按照自定义的组织策略二次处理该结果
...
断断续续的封城，路上的车子少了许多，连月光都变的好了，晚上就跟白昼一般，我却愈发看不清前方的路。
